...
static void GPS_Interrupt_Config(void)
{
  ...
  NVIC_PriorityGroupConfig(NVIC_PriorityGroup_2);
	...
	NVIC_InitStructure.NVIC_IRQChannel =  58 ;
...
static void GPS_DMA_Config(void)
{
		...
		/*内存地址自增*/
		DMA_InitStructure.DMA_MemoryInc = DMA_MemoryInc_Enable;	  //之前是8
		...
		/*优先级：中*/	
		DMA_InitStructure.DMA_Priority = DMA_Priority_Medium;////改成了LOW
    ...
    DMA_ITConfig(GPS_DMA_CHANNEL,DMA_IT_TC,ENABLE);  //配置DMA发送完成后产生中断DMA_IT_HT
